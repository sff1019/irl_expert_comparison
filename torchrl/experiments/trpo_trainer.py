...
from torchrl.experiment import IRLTrainer
...
def surrogate_loss(policy, all_obs, all_actions, all_adv, old_dist,
    ...
    if policy_ent > 0:
        entropy = Variable(
            torch.from_numpy(new_dist.entropy()).type(torch.FloatTensor))
...
class IRLTRPOTrainer(IRLTrainer):
    """
    ...
    def __init__(self,
        ...
        """
        super(IRLTRPOTrainer, self).__init__(**kwargs)
    ...
    def optimize_policy(self, itr, samples_data):
        ...
        action_var = Variable(
            torch.from_numpy(all_actions).type(torch.FloatTensor))
        ...
        surr_loss = surrogate_loss(self.policy, obs_var, action_var, adv_var,
                                   old_dist_var, self.pol_ent_wt)
        surr_loss.backward()
        ...
        def Fx(vector):
            if self.use_finite_diff_hvp:
                hvp = finite_diff_hvp(kl_divergence,
                                      obs_var,
                                      old_dist_var,
                                      self.policy,
                                      flat_kl_grad,
                ...
                hvp = pearlmutter_hvp(kl_divergence, obs_var, old_dist_var,
                                      self.policy, vector)
        ...
        if self.use_line_search:
            ...
            def barrier_func(x):
                ...
                surr_loss = surrogate_loss(self.policy, obs_var, action_var,
                                           adv_var, old_dist_var,
                                           self.pol_ent_wt)
                kl_div = kl_divergence(self.policy, obs_var, old_dist_var)
